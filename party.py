...
def is_mel(name, email):
    ...
    True
    >>> is_mel('Diana', 'diana@heb.com')
    False
    >>> is_mel('Joe', 'mel@ubermelon.com')
    True
    >>> is_mel('Mel Melitpolski', 'm@uber.com')
    True
...
def most_and_least_common_type(treats):
    ...
    If there is only one type in the list, it populates 'most' and 'least'
    ...
    if there is a tie in most or least, because the tie is within a dictionary, output could be either option

    
    If passed an empty list, output will be the tuple of None.
